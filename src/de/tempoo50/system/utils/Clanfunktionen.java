...
public class Clanfunktionen {
		...
		for (String uuid : list) {
			...
			if (pl.isOnline()) {
				Bukkit.getPlayer(UUID.fromString(uuid))
						.sendMessage("§8[§e" + "Clan-Chat" + "§8] §e" + player.getName() + "§8: §7" + msg);
			}
		}
	...
	public static void msgisonline(OfflinePlayer p, String clan) {
		...
		for (String uuid : list) {
			...
			if (pl.isOnline()) {
				Bukkit.getPlayer(UUID.fromString(uuid))
						.sendMessage("§8[§e" + "Clan" + "§8] " + ClanUtil.Spielergehtonline(p.getName()));
			}
		}
		List<String> listm = ClanManager.getMember(clan);
		for (String uuid : listm) {
	...
	public static void msgisoffline(OfflinePlayer p, String clan) {
		...
		for (String uuid : list) {
			...
			if (pl.isOnline()) {
				Bukkit.getPlayer(UUID.fromString(uuid))
						.sendMessage("§8[§e" + "Clan" + "§8] " + ClanUtil.Spielergehtoffline(p.getName()));
			}
		}
		List<String> listm = ClanManager.getMember(clan);
		for (String uuid : listm) {
	...
	public static void Playergejoint(OfflinePlayer player, String clan) {
		...
		for (String uuid : list) {
			...
			if (pl.isOnline()) {
				Bukkit.getPlayer(UUID.fromString(uuid))
						.sendMessage(ClanUtil.prefix() + " " + ClanUtil.Spielergejoint(player.getName()));
			}
		}
		List<String> listm = ClanManager.getMember(clan);
		for (String uuid : listm) {
	...
	public static void Einladungabgelehnt(OfflinePlayer player, String clan) {
		...
		for (String uuid : list) {
			...
			if (pl.isOnline()) {
				Bukkit.getPlayer(UUID.fromString(uuid))
						.sendMessage(ClanUtil.prefix() + " " + ClanUtil.Einladungabgelehnt(player.getName()));
			}
		}
		List<String> listm = ClanManager.getMember(clan);
		for (String uuid : listm) {
