...
public class ClanUtil {
	...
	static File file = new File("plugins//ServerSystem//Clan//ClanConfig.yml");
	...
	public static String Spielergejoint(String name) {
		return config.getString("message.Spieler in Clan beigetreten").replaceAll("&", "§").replaceAll("%player%",
				name);
	}
	...
	public static String Einladungabgelehnt(String name) {
		return config.getString("message.Clan Einladung nicht angenommen").replaceAll("&", "§").replaceAll("%player%",
				name);
	}
