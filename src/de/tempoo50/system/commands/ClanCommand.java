	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("info")) {
				if (!(ClanPlayer.getClan(p) == null)) {
					...
					p.sendMessage("§eKürzel§7: §6" + ClanManager.getPrefix(ClanPlayer.getClan(p)));
					p.sendMessage("§eClanleaders§7:");
					for (String uuid : ClanManager.getAdmins(ClanPlayer.getClan(p))) {
						p.sendMessage("§7- §8" + Bukkit.getOfflinePlayer(UUID.fromString(uuid)).getName());
					}
					p.sendMessage("§eClanmitglieder:");
					for (String uuid : ClanManager.getMember(ClanPlayer.getClan(p))) {
						p.sendMessage("§7- §8" + Bukkit.getOfflinePlayer(UUID.fromString(uuid)).getName());
					}
					p.sendMessage("§e§m---------<" + ClanUtil.prefix() + "§e>§m---------");
				} else {
			...
			if (args[0].equalsIgnoreCase("delete")) {
					...
					if ((ClanPlayer.getClan(Bukkit.getOfflinePlayer(player)) == null)) {
						if (ClanPlayer.isadmin(p, ClanPlayer.getClan(p))) {
							if (Bukkit.getOfflinePlayer(player).hasPlayedBefore()) {
								ClanPlayer.invitePlayer(Bukkit.getOfflinePlayer(player), ClanPlayer.getClan(p));
								p.sendMessage(ClanUtil.prefix() + " " + ClanUtil.Spielereingeladen());
								if (Bukkit.getOfflinePlayer(player).isOnline()) {
									Bukkit.getPlayer(player).sendMessage(ClanUtil.prefix() + " "
											+ ClanUtil.Einladungerhalten(ClanPlayer.getClan(p)));
									Bukkit.getPlayer(player)
											.sendMessage(ClanUtil.prefix() + " " + ClanUtil.joinoderdeny());
								}
							} else {
				...
				if (!(ClanPlayer.getClan(p) == null)) {
					if (ClanPlayer.isadmin(p, ClanPlayer.getClan(p))) {
						if (ClanPlayer.getClan(p)
								.equalsIgnoreCase(ClanPlayer.getClan(Bukkit.getOfflinePlayer(player)))) {
							ClanPlayer.leaveClan(Bukkit.getOfflinePlayer(player), ClanPlayer.getClan(p));
				...
				if (!(ClanPlayer.getClan(p) == null)) {
					if (ClanPlayer.isadmin(p, ClanPlayer.getClan(p))) {
						if (ClanPlayer.getClan(p).equalsIgnoreCase(ClanPlayer.getClan(player))) {
							ClanManager.promotePlayer(player, ClanPlayer.getClan(p));
		...
		if (args.length == 3) {
			...
			if (args[0].equalsIgnoreCase("create")) {
				...
				Integer amount = (Integer) e.get("Clan.Create");
				if ((prefix.length() >= 3) && (prefix.length() <= 7)) {

					if(!plugin.econ.hasAccount(p)) {
						
						p.sendMessage(MessagesUtil.KONTONOP);
						return true;
						
					}
					
					if(!plugin.econ.has(p, amount)) {
						
						p.sendMessage(MessagesUtil.PAYWARN1 + amount + " " + plugin.econ.currencyNameSingular() + MessagesUtil.PAYWARN2);
						
					}else {

						EconomyResponse withdraw = plugin.econ.withdrawPlayer(p.getName(), amount);
						
						if(withdraw.type != ResponseType.SUCCESS) {
							
							p.sendMessage(withdraw.errorMessage);
					...
					p.sendMessage(ClanUtil.prefix() + " "
							+ "§7Der §cTag §7darf nur zwischen 3 und 7 Buchstaben haben§8.");
					return false;
				}
